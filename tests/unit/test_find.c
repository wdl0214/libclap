...
void test_find_option_null_parser(void) {
    clap_argument_t *result = clap_find_option(NULL, "help", true);
...
void test_find_long_option_single(void) {
    clap_parser_t *parser = create_parser_with_options();
    clap_argument_t *expected = clap_add_argument(parser, "--help");
    
    clap_argument_t *result = clap_find_option(parser, "help", true);
...
void test_find_long_option_multiple(void) {
    clap_parser_t *parser = create_parser_with_options();
    clap_argument_t *help = clap_add_argument(parser, "--help");
    clap_argument_t *version = clap_add_argument(parser, "--version");
    clap_argument_t *output = clap_add_argument(parser, "--output");
    
    TEST_ASSERT_EQUAL_PTR(help, clap_find_option(parser, "help", true));
...
void test_find_long_option_case_sensitive(void) {
    ...
    result = clap_find_option(parser, "Help", true);
    TEST_ASSERT_NOT_NULL(result);
...
void test_find_short_option_single(void) {
    clap_parser_t *parser = create_parser_with_options();
    clap_argument_t *expected = clap_add_argument(parser, "-h");
    
    clap_argument_t *result = clap_find_option(parser, "h", false);
...
void test_find_short_option_multiple(void) {
    clap_parser_t *parser = create_parser_with_options();
    clap_argument_t *h = clap_add_argument(parser, "-h");
    clap_argument_t *v = clap_add_argument(parser, "-v");
    clap_argument_t *o = clap_add_argument(parser, "-o");
    
    TEST_ASSERT_EQUAL_PTR(h, clap_find_option(parser, "h", false));
...
void test_find_short_option_case_sensitive(void) {
    ...
    result = clap_find_option(parser, "H", false);
    TEST_ASSERT_NOT_NULL(result);
...
void test_find_option_with_both_forms(void) {
    clap_parser_t *parser = create_parser_with_options();
    clap_argument_t *expected = clap_add_argument(parser, "--help/-h");
    
    /* Should find by long option */
    clap_argument_t *result1 = clap_find_option(parser, "help", true);
    TEST_ASSERT_NOT_NULL(result1);
    TEST_ASSERT_EQUAL_PTR(expected, result1);
    
    /* Should find by short option */
    clap_argument_t *result2 = clap_find_option(parser, "h", false);
...
void test_find_option_with_multiple_combined(void) {
    ...
    TEST_ASSERT_EQUAL_PTR(help, clap_find_option(parser, "help", true));
    TEST_ASSERT_EQUAL_PTR(help, clap_find_option(parser, "h", false));
...
void test_find_option_first_match(void) {
    ...
    clap_parser_t *parser = create_parser_with_options();
    clap_argument_t *first = clap_add_argument(parser, "--help");
    /* This would normally be prevented, but we test the behavior */
...
void test_find_option_fast_basic(void) {
    ...
    clap_argument_t *result = clap_find_option_fast(parser, "help", true);
...
void test_find_option_fast_short_option(void) {
    ...
    clap_argument_t *result = clap_find_option_fast(parser, "h", false);
...
void test_find_option_fast_null_parser(void) {
    clap_argument_t *result = clap_find_option_fast(NULL, "help", true);
    TEST_ASSERT_NULL(result);
}
...
void test_find_option_fast_returns_same_as_slow(void) {
    clap_parser_t *parser = create_parser_with_options();
    clap_add_argument(parser, "--help/-h");
    ...
    for (int i = 0; i < 10; i++) {
        const char *test_names[] = {"help", "h", "version", "V", "output", "o", "verbose", "v", "nonexistent"};
        bool is_long[] = {true, false, true, false, true, false, true, false, true};
